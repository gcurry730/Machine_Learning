...
def avg_value(data_set, attribute):
    '''
    ========================================================================================================
    ...
    for i in range(len(data_set)):  
        if data_set[i][attribute] is not None: 
            SUM = SUM + data_set[i][attribute]
            count = count +1
    avg = SUM/float(count)
    return avg
...
def ID3(data_set, attribute_metadata, numerical_splits_count, depth):
    ...
    if depth > 0: 
        ...
        if entropy(data_set) < Theta:
            ...
            n0.mode = mode(data_set)

            
            # make a deep copy with filled in values (the average of that attribute) for None at the mode          
            deep_data_set = deepcopy(data_set)
            for i in range (len(deep_data_set)):
                if attribute_metadata[best_attr[0]]['is_nominal'] is False:
                    if deep_data_set[i][best_attr[0]] is None:
                        n0.avg = avg_value(deep_data_set, best_attr[0]) # store this avg value in case I need to use it in classify() 
                        deep_data_set[i][best_attr[0]] = n0.avg
                        #print "I put in the value: " + str(n0.avg) + " which should be around " + str(deep_data_set[i+1][best_attr[0]])
            ...
            if attribute_metadata[best_attr[0]]['is_nominal'] is True:
                ...
                new_data_set = split_on_nominal(deep_data_set, best_attr[0])
                ...
                new_data_set = split_on_numerical(deep_data_set, best_attr[0], best_attr[1])
                for i in range(len(new_data_set)):
